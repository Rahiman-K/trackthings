...
router.post('/stop/:sessionId', (req, res) => {
  ...
  // If still running, add the remaining time
...
// Get active session (currently running timer)
router.get('/active', (req, res) => {
  const session = db.prepare(
    "SELECT ts.*, t.title as task_title FROM time_sessions ts JOIN tasks t ON ts.task_id = t.id WHERE ts.status IN ('running', 'paused') ORDER BY ts.started_at DESC LIMIT 1"
  ).get();
